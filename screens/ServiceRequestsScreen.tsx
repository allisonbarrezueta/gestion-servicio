import React, { useEffect, useState } from "react";
import { StyleSheet, Text, TouchableOpacity, View, ActivityIndicator, FlatList, SafeAreaView } from "react-native";
import dayjs from "dayjs";
import { StackScreenProps } from "@react-navigation/stack";

/* Models */
import Request from "./../models/Request";

/* Types */
import { TabOneParamList } from "./../types";

const OrderItem = ({ navigation, request }: { request: Request; navigation: any }) => {
    const date = dayjs(request.$attributes.date);

    return (
        <TouchableOpacity
            onPress={() => navigation.navigate("Solicitud de Servicio", { request })}
            style={{
                borderWidth: 1,
                borderColor: "#99d1e0",
                borderRadius: 10,
                flexDirection: "column",
                paddingHorizontal: 12,
                marginBottom: 24,
                paddingVertical: 12,
            }}>
            <View style={{ marginBottom: 18 }}>
                <Text style={{ fontSize: 16, color: "#0d394e", fontWeight: "600" }}>{request.$relations.service.$attributes.name}</Text>
            </View>

            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                <Text style={{ fontSize: 16, color: "#0d394e", fontWeight: "600" }}>{date.format("YYYY-MM-DD HH[h]mm")}</Text>
            </View>
        </TouchableOpacity>
    );
};

type Props = StackScreenProps<TabOneParamList, "Solicitudes de Servicio">;

export default function ServiceRequestsScreen({ route, navigation }: Props) {
    const [requests, setRequests] = useState<Request[]>([]);
    const [loading, setLoading] = useState(false);
    const category = route.params.category;

    useEffect(() => {
        const getRequest = async () => {
            setLoading(true);
            try {
                const results = await Request.$query()
                    .filter("category_id", "=", category.$attributes.id)
                    .filter("date", ">", dayjs())
                    .sortBy("created_at", "desc")
                    .with(["category", "service", "user", "media"])
                    .search();
                setRequests(results);
            } catch (error) {
                console.log(error);
            } finally {
                setLoading(false);
            }
        };

        getRequest();
    }, [category]);

    return (
        <SafeAreaView style={styles.container}>
            <View style={{ width: "100%", flex: 1, paddingVertical: 22 }}>
                <Text style={{ fontSize: 24, fontWeight: "600", marginBottom: 18, color: "#4c5763" }}>
                    Solicitudes de {category.$attributes.name}:
                </Text>

                {loading ? (
                    <ActivityIndicator size="large" color="#99d1e0" />
                ) : (
                    <FlatList
                        style={{ width: "100%" }}
                        data={requests}
                        renderItem={({ item }: { item: Request }) => <OrderItem request={item} navigation={navigation} />}
                        keyExtractor={(item) => `${item.$attributes.id}`}
                    />
                )}
            </View>
        </SafeAreaView>
    );
}

const styles = StyleSheet.create({
    container: {
        flex: 1,
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: "white",
        paddingHorizontal: 22,
    },
    title: {
        fontSize: 20,
        fontWeight: "500",
        color: "#838a92",
        marginTop: 12,
    },
    separator: {
        marginVertical: 30,
        height: 1,
        width: "80%",
    },
    input: {
        borderColor: "#ced2d8",
        borderWidth: 1,
        height: 54,
        width: "100%",
        color: "black",
        paddingHorizontal: 12,
        marginBottom: 24,
        fontSize: 18,
        borderRadius: 10,
    },
});
